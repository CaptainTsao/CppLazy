...
#ifndef CPPLAZY_INCLUDE_LAZY_MAP_HPP_
#define CPPLAZY_INCLUDE_LAZY_MAP_HPP_
...
class Map final : public internal::BasicIteratorView<internal::MapIterator<Iterator, Function>> {
 public:
  ...
  LZ_CONSTEXPR_CXX_20 Map(Iterator begin, Iterator end, Function function) :
      internal::BasicIteratorView<iterator>(iterator(std::move(begin), function),
                                            iterator(std::move(end), function)) {}
...
LZ_NODISCARD LZ_CONSTEXPR_CXX_20 Map<Iterator, Function>
map_range(Iterator begin, Iterator end, Function function) {
...
map(Iterable &&iterable, Function function) {
  return map_range(internal::begin(std::forward<Iterable>(iterable)),
                   internal::end(std::forward<Iterable>(iterable)),
                   std::move(function));
}

}//namespace lz
#endif //CPPLAZY_INCLUDE_LAZY_MAP_HPP_
