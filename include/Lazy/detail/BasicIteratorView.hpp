//
// Created by rcao on 2021/7/2.
//

#ifndef CPPLAZY_INCLUDE_LAZY_DETAIL_BASICITERATORVIEW_HPP_
#define CPPLAZY_INCLUDE_LAZY_DETAIL_BASICITERATORVIEW_HPP_
...
ToStringImplSpecialized(std::string &result, Iterator begin, Iterator end, const std::string delimiter,
  ...
  std::for_each(begin, end, [&oss, &delimiter](const ValueType<Iterator> &iter) {
    oss << iter << delimiter;
  });
  result = oss.str();
}
...
#else
std::string doMakeString(const Iterator &b,
                         const Iterator &e,
                         const std::string &delimiter,
                         std::true_type /* isChar */) {
...
class BasicIteratorView {
 ...
 private:
  ...
  template<class Container, class It = LzIterator, EnableIf<HasReserve<Container>::value && IsForward<It>::value,
                                                            bool> = true>
 ...
 public:
  ...
  template<class KeySelectorFunc, class Hasher = std::hash<KeyType<KeySelectorFunc>>,
      ...
      class Allocator = std::allocator<std::pair<const KeyType<KeySelectorFunc>, value_type>>>
  LZ_NODISCARD LZ_CONSTEXPR_CXX_20 std::unordered_map<KeyType<KeySelectorFunc>,
                                                      value_type,
                                                      Hasher,
                                                      KeyEquality,
                                                      Allocator>
...
#endif //CPPLAZY_INCLUDE_LAZY_DETAIL_BASICITERATORVIEW_HPP_
