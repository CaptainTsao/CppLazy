...
#ifndef LAZY_DETAIL_ZIPITERATOR_HPP_
#define LAZY_DETAIL_ZIPITERATOR_HPP_
...
class ZipIterator {
 ...
 private:
  ...
  template<std::size_t... I>
  LZ_CONSTEXPR_CXX_20 void increment(IndexSequence<I...>) const {
    const int expand[] = {(++std::get<I>(iterators_), 0)...};
    static_cast<void>(expand);
  }

  template<std::size_t... I>
  LZ_CONSTEXPR_CXX_20 void decrement(IndexSequence<I...>) const {
    const int expand[] = {(--std::get<I>(iterators_), 0)...};
    static_cast<void>(expand);
  }

  template<std::size_t... I>
  LZ_CONSTEXPR_CXX_20 void plus_is(IndexSequence<I...>, const difference_type diff_type) {
    const int expand[] = {((std::get<I>(iterators_) += diff_type), 0)...};
    static_cast<void>(expand);
  }

  template<std::size_t... I>
  LZ_CONSTEXPR_CXX_20 void min_is(IndexSequence<I...> is, const difference_type diff_type) {
    put_is(is, -diff_type);
  }

 public:
  LZ_CONSTEXPR_CXX_20 explicit ZipIterator(std::tuple<Iterators...> iterators)
  ...
  LZ_CONSTEXPR_CXX_20 ZipIterator& operator+=(const difference_type offset) {
    plus_is(MakeIndexSequenceForThis(), offset);
    return *this;
  }
  ...
  LZ_CONSTEXPR_CXX_20 ZipIterator& operator-=(const difference_type offset) {
    min_is(MakeIndexSequenceForThis(), offset);
    return *this;
  }
...
#endif //LAZY_DETAIL_ZIPITERATOR_HPP_
